...
-   Email [bs3141\@columbia.edu](mailto:bs3141@columbia.edu){.email} for
    general questions

-   Email
    [zw2899\@cumc.columbia.edu](mailto:zw2899@cumc.columbia.edu){.email}
    for questions related to `cmest_multistate`
...
[^1]: rb: the regression-based approach; wb: the weighting-based
    approach; iorw: the inverse odds ratio weighting approach; ne: the
    natural effect model; msm: the marginal structural model; gformula:
    the g-formula approach.

[^2]: Y denotes the outcome, A denotes the exposure, M denotes the
    mediator(s) and C denotes the exposure-outcome confounder(s), the
    exposure-mediator confounder(s) and the mediator-outcome
    confounder(s) not affected by the exposure.

[^3]: continuous A is not supported when C is not empty; otherwise, it
    is supported.

[^4]: continuous A is not supported when C is not empty; otherwise, it
    is supported.
...
[^7]: closed-form parameter function estimation only supports a single
    mediator.

[^8]: delta method inference is available only when closed-form
    parameter function estimation is used.

[^9]: marginal effects are estimated when direct counterfactual
    imputation estimation is used.
