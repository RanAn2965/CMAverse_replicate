...
#'
#' \code{cmest_rb} is used to implement \emph{the regression-based approach}
...
#'
#' @export
cmest_rb <- function(data = NULL, outcome = NULL, event = NULL, exposure = NULL, mediator = NULL, EMint = NULL, basec = NULL, yreg = NULL, mreg = NULL, estimation = "imputation", inference = "bootstrap", astar = NULL, a = NULL, mval = NULL, basecval = NULL, yval = NULL, nboot = 200, boot.ci.type = "per", casecontrol = FALSE, yrare = NULL, yprevalence = NULL, multimp = FALSE, args_mice = NULL) {
  
  # =========================================================================
  # ARCHITECTURAL LOGIC: ENTRY POINT & SESSION INITIALIZATION
  # =========================================================================
  # Capture and store the user's function call to enable reproducible updates or print summaries
  cl <- match.call()
  out <- list(call = cl)
  
  # =========================================================================
  # INPUT & DESIGN VALIDATION GUARDRAILS
  # =========================================================================
  ...
  # Ensure all requested exposure, mediator, outcome, and confounder variables actually exist in the data columns
  ...
  # Missing Value (NA) Screening: If multiple imputation is FALSE, halt immediately on missing records to avoid math bias
  ...
  # 2. Method Names Standardizations
  ...
  # 3. Structural Capabilities Guardrails
  if (!estimation %in% c("paramfunc", "imputation")) stop("Select estimation from 'paramfunc', 'imputation'")
  
  # Parametric Closed-Form Calculations restrictions: Must have a single mediator and character formulas
  if (estimation == "paramfunc") {
    if (!is.character(yreg) | length(yreg) != 1 | !yreg %in% c("linear", "logistic", "loglinear", "poisson", "quasipoisson", "negbin", "coxph", "aft_exp", "aft_weibull")) stop(
      "When estimation = 'paramfunc', select yreg from 'linear', 'logistic', 'loglinear', 'poisson', 'quasipoisson', 'negbin', 'coxph', 'aft_exp', 'aft_weibull'")
    if (length(mediator) > 1) stop("The parameter function estimation only supports a single mediator")
    if (!is.character(mreg[[1]]) | length(mreg[[1]]) != 1 | !mreg[[1]] %in% c("linear", "logistic", "multinomial")) stop(
  ...
  # 5. Case-Control Study Specific Adjustments
  ...
  if (casecontrol) {
    ...
    # Imputation-based potential outcomes require yprevalence weights to correct for case oversampling bias
  ...
  # 8. Multiple Imputation Preprocessing
  ...
  # =========================================================================
  # ARCHITECTURAL LOGIC: LEXICAL SCOPING OVERRIDES FOR REGRESSION COUPLING
  # =========================================================================
  # Dynamically bind the lexical scoping environment of the fitting engine to this executing scope.
  # This avoids copying data / lists of variables, allowing 'regrun_rb' to directly read parent params.
  ...
  # =========================================================================
  # BOOTSTRAP PROGRESS BAR INITIALIZATION
  # =========================================================================
  if (inference == "bootstrap") {
    env <- environment()
    counter <- 0
  ...
  # =========================================================================
  # ARCHITECTURAL LOGIC: LEXICAL SCOPING OVERRIDES FOR EFFECT CALCULATIONS
  # =========================================================================
  # Bind estimation and bootstrap functions to this environment.
  # Estinf_rb inherits variables and fitted models automatically.
...
# =========================================================================
# HELPER FUNCTION: REGRESSION COMPILATION & FITTING ENGINE
# =========================================================================
regrun_rb <- function() {
  
  # 1. Mediator Model Construction & Fitting
  ...
  for (p in 1:length(mreg)) {
    ...
    if (is.character(mreg[[p]])) {
      # Verify binary scale mapping for Log-Linear fits
      ...
      if (!mreg[[p]] %in% c("linear", "logistic", "loglinear", "poisson", "quasipoisson", "negbin", "multinomial", "ordinal")) stop(
        paste0("Select character mreg[[", p, "]] from 'linear', 'logistic', 'loglinear', 'poisson', 'quasipoisson', 'negbin', 'multinomial', 'ordinal'"))
      
      # Build formula string: Mediator ~ Exposure + Baseline Confounders
      mediator_formula <- paste0(mediator[p], "~", paste(c(exposure, basec), collapse = "+"))
      
      # Route formulas to respective statistical frameworks
      switch(mreg[[p]], 
    ...
    } else if (!(identical(class(mreg[[p]]), "lm") | identical(class(mreg[[p]]), c("glm", "lm")) | identical(class(mreg[[p]]), c("negbin", "glm", "lm")) | identical(class(mreg[[p]]), c("multinom", "nnet")) | identical(class(mreg[[p]]), c("gam", "glm", "lm")) | identical(class(mreg[[p]]), "polr"))) {
      ...
      if (!(family_mreg$family %in% c("gaussian", "inverse.gaussian", "poisson", "quasipoisson", "Gamma", "binomial", "multinom") | startsWith(family_mreg$family, "Negative Binomial") | startsWith(family_mreg$family, "Ordered Categorical"))) stop(paste0("Unsupported mreg[[", p, "]]"))
    } else {
      # For custom pre-fitted model objects, enforce that they only regress on Exposure and Confounders
  ...
  # 2. Outcome Model Construction & Fitting
  ...
  if (is.character(yreg)) {
    ...
    if (!yreg %in% c("linear", "logistic", "loglinear", "poisson", "quasipoisson", "negbin", "multinomial", "ordinal", "coxph", "aft_exp", "aft_weibull")) stop(
      paste0("Select character yreg from 'linear', 'logistic', 'loglinear', 'poisson', 'quasipoisson', 'negbin', 'multinomial', 'ordinal', 'coxph', 'aft_exp', 'aft_weibull'"))
    
    # Logistic/Cox parametric models violate counterfactual derivations unless outcome is a rare event
    ...
    # Interaction terms mapping: If EMint is active, construct joint interaction variables for exposure and mediators
    int.terms <- switch(EMint + 1, "1" = NULL, "2" = paste(exposure, mediator, sep = "*"))
    
    # Build complete formula: Outcome ~ Exposure + Mediator(s) + Exposure*Mediator(s) + Baseline Confounders
    outcome_formula <- paste0(outcome, "~", paste(c(exposure, mediator, int.terms, basec), collapse = "+"))
    
    # Adjust formulas for Survival contexts incorporating right-censored time-to-event outcomes
    if (yreg %in% c("coxph","aft_exp","aft_weibull")) {
      if (!is.null(event)) {
        outcome_formula <- paste(paste0("Surv(", outcome, ", ", event, ")"), strsplit(outcome_formula, split = "~")[[1]][2], sep = " ~ ")
      } else outcome_formula <- paste(paste0("Surv(", outcome, ")"), strsplit(outcome_formula, split = "~")[[1]][2], sep = " ~ ")
    }
    
    # Route outcome formulas to respective statistical engines
    switch(yreg, 
           linear = yreg <- eval(bquote(glm(formula = .(as.formula(outcome_formula)), family = gaussian(), data = .(data)))), 
           logistic = yreg <- eval(bquote(glm(formula = .(as.formula(outcome_formula)), family = binomial(), data = .(data)))), 
           loglinear = yreg <- eval(bquote(glm(formula = .(as.formula(outcome_formula)), family = poisson(), data = .(data)))), 
           poisson = yreg <- eval(bquote(glm(formula = .(as.formula(outcome_formula)), family = poisson(), data = .(data)))), 
           quasipoisson = yreg <- eval(bquote(glm(formula = .(as.formula(outcome_formula)), family = quasipoisson(), data = .(data)))), 
           negbin = yreg <- eval(bquote(MASS::glm.nb(formula = .(as.formula(outcome_formula)), data = .(data)))), 
           multinomial = yreg <- eval(bquote(nnet::multinom(formula = .(as.formula(outcome_formula)), data = .(data), trace = FALSE))), 
           ordinal = yreg <- eval(bquote(MASS::polr(formula = .(as.formula(outcome_formula)), data = .(data)))), 
           coxph = yreg <- eval(bquote(survival::coxph(formula = .(as.formula(outcome_formula)), data = .(data)))), 
           aft_exp = yreg <- eval(bquote(survival::survreg(formula = .(as.formula(outcome_formula)), dist = "exponential", data = .(data)))), 
           aft_weibull = yreg <- eval(bquote(survival::survreg(formula = .(as.formula(outcome_formula)), dist = "weibull", data = .(data)))))
  } else if (!(identical(class(yreg), "lm") | identical(class(yreg), c("glm", "lm")) | identical(class(yreg), c("negbin", "glm", "lm")) | identical(class(yreg), c("multinom", "nnet")) | identical(class(yreg), c("gam", "glm", "lm")) | identical(class(yreg), "polr") | identical(class(yreg), "coxph") | identical(class(yreg), "survreg"))) {
    ...
    if (!(family_yreg$family %in% c("gaussian", "inverse.gaussian", "quasi", "poisson", "quasipoisson", "Gamma", "binomial", "quasibinomial", "multinom", "ziplss") | startsWith(family_yreg$family, "Negative Binomial") | startsWith(family_yreg$family, "Zero inflated Poisson") | startsWith(family_yreg$family, "Ordered Categorical"))) stop("Unsupported yreg")
  } else {
  ...
  # 3. Delta Method Adjustment for Complex Case-Control Survey Designs
  # If running Delta calculations with binary case-control outcomes, intercept standard GLMs
  # and re-structure models using 'survey::svyglm' / 'svymultinom' to get robust standard errors.
  if (inference == "delta" && casecontrol && !is.null(yprevalence)) {
    yreg <- suppressWarnings(eval(bquote(survey::svyglm(formula = .(formula(yreg)), family = .(family(yreg)), design = survey::svydesign(~ 1, data = .(data))))))
  }
  if (inference == "delta" && casecontrol && !is.null(yprevalence)) {
    if (inherits(mreg[[1]], "glm")) mreg[[1]] <- suppressWarnings(eval(bquote(survey::svyglm(formula = .(formula(mreg[[1]])), family = .(family(mreg[[1]])), design = survey::svydesign(~ 1, data = .(data))))))
    if (inherits(mreg[[1]], "multinom")) mreg[[1]] <- suppressWarnings(eval(bquote(svymultinom(formula = .(formula(mreg[[1]])), data = .(data)))))
  }
...
# =========================================================================
# HELPER FUNCTION: INFERENCE & ESTIMATION CONTROLLER (estinf_rb)
# =========================================================================
estinf_rb <- function() {
  
  # 1. Enforce acceptable standard input classes
  allvar <- c(outcome, event, exposure, mediator, basec)
  for (i in 1:length(allvar)) if (!(is.numeric(data[, allvar[i]]) | is.factor(data[, allvar[i]]) | is.character(data[, allvar[i]]))) stop(paste0("The variable ", allvar[i], " must be numeric, factor or character"))
  
  out <- list()
  
  # 2. Extract model class metrics and structures for internal routing
  ...
  # 3. Resolve Reference Levels for continuous vs categorical exposures
  ...
  # Find yval for categorical outcome levels
  if ((is_glm_yreg && (family_yreg$family %in% c("binomial", "quasibinomial", "multinom") | startsWith(family_yreg$family, "Ordered Categorical"))) | is_multinom_yreg | is_polr_yreg) {
  ...
  # Set reference defaults for mediators
  for (i in 1:length(mediator)) {
    if ((is_glm_mreg[i] && ((family_mreg[[i]]$family %in% c("binomial", "multinom")) | startsWith(family_mreg[[i]]$family, "Ordered Categorical"))) | is_multinom_mreg[i] | is_polr_mreg[i]) {
  ...
  # 4. Create conditional value profiles for parametric setups
  if (estimation == "paramfunc") {
    if (length(basec) != 0) {
      if (!is.null(basecval)) {
      ...
      for (i in 1:length(basec)) {
        if (is.factor(data[, basec[i]]) | is.character(data[, basec[i]])) {
          ...
          if (is.null(basecval[[i]])) {
            ...
            basecval[[i]] <- unname(colMeans(as.matrix(model.matrix(as.formula(paste0("~", basec[i])), data = model.frame(~., data = c_data, na.action = na.pass))[, -1]), na.rm = TRUE))
            rm(c_data)
  ...
  # =========================================================================
  # EST_RB DYNAMIC BINDING & EXECUTION SWITCHBOARD
  # =========================================================================
  # Override 'est_rb' helper's environment so that it dynamically loads
  # all extracted parameters, data shapes, and variables from this execution context.
  environment(est_rb) <- environment()
  
  if (!multimp) {
    # Point Estimates evaluation
    est <- est_rb(data = data, indices = NULL, outReg = TRUE)
    effect.pe <- est$est
    n_effect <- length(effect.pe)
    out$reg.output <- est$reg.output
    
    # Pathway A: Bootstrapping SE & CIs
    if (inference == "bootstrap") {
    ...
    # Pathway B: Closed-Form Delta Method SE & CIs
    ...
    # Parallel Imputations Loop via MICE
    data_imp <- complete(do.call(mice, args_mice), action = "all")
    m <- length(data_imp)
    
    # Fit and pool estimates across multiple imputed datasets
    est_imp <- lapply(1:m, function(x) est_rb(data = data_imp[[x]], indices = NULL, outReg = TRUE))
    ...
    # Imputed Bootstrap Routine: captures sampling + imputation variance
    if (inference == "bootstrap") {
      boot.step <- function(data = NULL, indices = NULL) {
        ...
        return(colMeans(do.call(rbind, lapply(1:m, function(x) est_rb(data = data_imp[[x]], outReg = FALSE)))))
      }
    ...
    # Imputed Delta Method SE & Pooling
    } else if (inference == "delta") {
      environment(inf.delta) <- environment()
      se_imp <- do.call(rbind, lapply(1:m, function(x) inf.delta(data = data_imp[[x]], yreg = est_imp[[x]]$reg.output$yreg, mreg = est_imp[[x]]$reg.output$mreg[[1]])))
      
      # Combine Standard Errors using Rubin's Rules
  ...
  # =========================================================================
  # EFFECTS RATIO / DIFFERENCE SCALE STANDARDIZATION & EXTRAS POOLING
  # =========================================================================
  if ((is_lm_yreg | is_glm_yreg) && (family_yreg$family %in% c("gaussian", "inverse.gaussian", "Gamma", "quasi"))) {
    if (inference == "bootstrap") effect.se <- sapply(1:n_effect, function(x) sd(boots$t[, x], na.rm = TRUE))
    
    # Map effect names on the difference scale (including 4-way decomps if active)
    if (EMint) effect_name <- c("cde", "pnde", "tnde", "pnie", "tnie", "te", "intref", "intmed", "cde(prop)", "intref(prop)", "intmed(prop)", "pnie(prop)", "pm", "int", "pe")
    if (!EMint) effect_name <- c("cde", "pnde", "tnde", "pnie", "tnie", "te", "pm")
  } else {
    # If the outcome is non-linear, convert variance and estimates back to the relative Ratio scale
    if (inference == "bootstrap") effect.se <- sapply(1:n_effect, function(x) ifelse(x <= 6, sd(exp(boots$t[, x])), sd(boots$t[, x])))
    if (inference == "delta" && estimation == "paramfunc") {
      # Apply delta method on log scale variables for the first 6 contrasts
      # effect.se[1:6] <- sapply(1:6, function(x) deltamethod(as.formula("~exp(x1)"), effect.pe[x], effect.se[x]^2))
    }
    
    # Exponential conversion of point estimates & intervals from log scale
    effect.pe[1:6] <- exp(effect.pe[1:6])
    effect.ci.low[1:6] <- exp(effect.ci.low[1:6])
    effect.ci.high[1:6] <- exp(effect.ci.high[1:6])
    
    # Map excess risk / rate ratio names for categorical outcomes (4-way decomposition)
    if (EMint) effect_name <- c("Rcde", "Rpnde", "Rtnde", "Rpnie", "Rtnie", "Rte", "ERcde", "ERintref", "ERintmed", "ERpnie", "ERcde(prop)", "ERintref(prop)", "ERintmed(prop)", "ERpnie(prop)", "pm", "int", "pe")
    if (!EMint) effect_name <- c("Rcde", "Rpnde", "Rtnde", "Rpnie", "Rtnie", "Rte", "pm")
  }
  
  names(effect.pe) <- names(effect.se) <- names(effect.ci.low) <- names(effect.ci.high) <- names(effect.pval) <- effect_name
